	...
	case FocusRunners:
		if len(m.Runners) == 0 {
			if m.IsRunnersLoading {
				m.Viewport.SetContent(fmt.Sprintf(" %s Fetching registered runners...", cellStatusIconStyle.Render(m.Spinner.View())))
			} else {
	...
	case FocusJobs:
		if len(m.JobQueue) == 0 || m.SelectedJobIndex >= len(m.JobQueue) {
			if m.IsJobQueueLoading || m.IsOrgSyncing {
				m.Viewport.SetContent(fmt.Sprintf(" %s Fetching active workflow jobs...", cellStatusIconStyle.Render(m.Spinner.View())))
			} else {
